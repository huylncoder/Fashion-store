...
const AddVoucher = (props) => {
    ...
    const handleOnChange = event => {
        ...
        // Check if the value is negative for amount
        if (name === 'amount' && numericValue < 0) {
            toast.error("Số lượng mã không được âm");
            return; // Prevent updating state if the value is negative
        }
